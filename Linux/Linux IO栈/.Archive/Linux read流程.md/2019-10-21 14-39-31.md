		...
		gfp_t gfp)
{
	...
	page_block = 0;
```
首先检查页面PG_private标志;若该标志已设置，则表明:(1)它是缓冲页面 (即该页面与一个buffer head链表关联)，且已经从磁盘上读取到内存中;(2)页面中的块在 磁盘上不是相邻的;这样的话，就跳转到318行，一次读取页面的一个块。
获取块大小blocksize(保存在page->mapping->host->inode->i_blkbits)，并且计算访问这 个页面所需的两个值:页面中的块数(173行)和页面的第一个块(即页内的第一个块相对于文 件起始位置的索引)。
block_in_file:page中的第一个block number。 last_block:page中最后一个block 大小。 last_block_in_file:文件最后一个block大小。 last_block最终值为本次page读操作的最后一个block大小。
