...
当前page和上一个page的物理 block不连续，把上一次的bio给提交，然后重新分配bio。
若当前page和上一个page，物理上连续，合并成新的bio后，返回更新后的bio，等待下一 次操作(316行)。
318~325行:若函数跳转到这里，页面中包含的块在磁盘上是不相邻的。若页面是最新的 (设置了PG_update标志)，调用unlock_page()来对页面解锁，否则就调用 block_read_full_page()来一次读取页面上的一个块。
mpage_bio_submit()函数，首先设置bio->bi_end_io的值为mpage_end_io_read()， 该函数是bio结束方法。当I/O数据传输结束时，它就立即被执行。假设没有I/O错误，该函数主 要设置页面描述符中的PG_uptodate标志，然后调用unlock_page()来解锁页面，并唤醒在事 件上等待的所有进程;最后调用bio_put()销毁bio描述符。在调用submit_bio()，设置数据 传输方向到bi_rw标志，更新per-CPU类型变量page_states以记录读扇区数;然后调用 generic_make_request()。
